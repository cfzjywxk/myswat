...
class MemoryRetriever:
    ...
    def _build_myswat_cli_context(self, project: dict | None, repo_path: str | None) -> str:
        ...
        return (
            ...
            f"- `{launcher} history -p {slug} --turns 50` — inspect recent raw turns with exact chronology\n"
            f"- `{launcher} search \"<query>\" -p {slug}` — search persisted project knowledge before guessing\n"
            f"- `{launcher} task <id> -p {slug}` — inspect one work item in detail when you know the task id\n"
            f"- `{launcher} chat -p {slug}` — resume interactive orchestration with the team\n"
            f"- `{launcher} work \"<requirement>\" -p {slug}` — start a new orchestrated workflow\n\n"
            "Useful commands inside `myswat chat`:\n"
            "- `/status` — show active work items and recent process flow\n"
            "- `/task <id>` — inspect one work item without leaving chat\n"
            "- `/agents` — list roles/models\n"
            "- `/history [n]` — inspect recent turns from the active chat session\n"
            "- `/work <requirement>` — start an orchestration run from chat\n\n"
            "## History Access\n\n"
            "Recent turns are persisted for this project. You are seeing the 10 most "
            "recent per role. If you need more context:\n"
            f"- `{launcher} history -p {slug} --turns 100` — inspect more raw turns\n"
            f"- `{launcher} status -p {slug} --details` — inspect workflow history, review loops, and handoffs\n"
        )
